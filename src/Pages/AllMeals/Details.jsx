...
const Details = () => {
    ...
    const { data: meal, isLoading: mealLoading, error: mealError, refetch: refetchMeal } = useQuery({
        queryFn: () => fetch(`https://hostel-server-seven.vercel.app/meals/${id}`).then(res => res.json()),
        queryKey: ["meal", id]
    });
    ...
    const { _id, title, category, price, rating, image, description, likes } = meal;

    const addToRequestMeal = (food) => {
        if (user && user.email) {
            const requestItem = {
                mealId: _id,
                email: user.email,
            ...
            axiosSecure.post('/requestedMeal', requestItem)
                .then(res => {
                    if (res.data.insertedId) {
                        Swal.fire({
                            position: "top-end",
                            icon: "success",
                            title: `${meal.title} added to your requested meal list`,
    ...
    const handleLike = async (id) => {
        try {
            await axiosSecure.patch(`/meals/likes/${id}`);
            refetchMeal();
        } catch (error) {
        ...
        <div className="container mx-auto p-4 flex justify-between">
            <div className="relative pt-20 pb-10 w-1/2 bg-base-100 shadow-xl overflow-hidden rounded-2xl mb-4">
                ...
                <div className="flex justify-center gap-7">
                    <button onClick={() => addToRequestMeal(meal)} className="btn btn-primary">Request meal</button>
